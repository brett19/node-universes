...
cluster.on('auth', function(username, password) {
  SocketAuth(log);
  SocketAuth.Authenticate(username, password, function(err, sessionid, signature) {
    ...
    log.info('Successful authentication for ' + username + ' (' + sessionid + ':' + signature + ')');
  });
