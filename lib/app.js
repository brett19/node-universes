...
function MmoApp(options, callback) {
  ...
  this.cluster.on('nodeLeft', function(uuid) {
    ...
    delete this.peerServices[uuid];
  }.bind(this));
  this.cluster.non('core/addService', function(client, args) {
    this.peerServices[client.uuid].push(args.id);
  }.bind(this));
  this.cluster.non('core/removeService', function(client, args) {
    var serviceIdx = this.peerServices[client.uuid].indexOf(args.id);
    if (serviceIdx >= 0) {
      this.peerServices[client.uuid].splice(serviceIdx, 1);
    }
  }.bind(this));
}
...
MmoApp.prototype.debugInfo = function() {
  return {
    ...
    peerServices: this.peerServices
  };
};
