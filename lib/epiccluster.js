...
_Client.prototype._handleData = function(data) {
  if (data.cmd === 'register') {
    ...
    this.send({
      cmd: 'nodelist',
      nodes: this.parent.nodeList()
    });
  } else if (data.cmd === 'nodelist') {
...
EpicCluster.prototype.msg = function(cmd, handler) {

};

// sends to everyone but me.
EpicCluster.prototype.send = function(cmd, args) {
  for (var i = 0; i < this.nodes.length; ++i) {
    this.nodes[i].send({
      cmd: 'x',
      sub: cmd,
